<?php
    error_reporting(0);
    include './model/report-sale.model.php'; 
...
<div class="content">
    <h1><i class="fa-solid fa-boxes-stacked"></i> รายงานเบิกสินค้า</h1>
    <hr>
    <div class="table-responsive" style="overflow-x: hidden;">
        <table class="table table-bordered table-sm" id="dataTable" width="100%">
            <thead>
                ...
                <th>ราคาขาย</th>
                <th>จำนวนที่เบิก</th>
                <th>ยอดเงินทั้งสิ้น</th>
                <th>ส่วนลด</th>
                <th>วันที่เบิกขาย</th>
                <th>ผู้ลงข้อมูล</th>
            </thead>
            <tbody>
                <?php foreach (ReportSaleModel::_main($start_date, $end_date) as $v) { ?>
                    <tr>
                        <td><?=$v['bill_id']?></td>
        ...
        <?php if (empty($_GET['start_date']) && empty($_GET['end_date'])) { ?>
            alertify.confirm(
                ...
                () => {
                    ...
                    window.location.href = `index?r=report-sale&start_date=${strFrom[0].value}&end_date=${strFrom[1].value}`;
                }, () => {}
            );
        <?php } ?>
    });
